...
func SendEventClosed(c tele.Context) error {
	return c.Send(locale.ResultEventClosed, tele.RemoveKeyboard, tele.NoPreview, tele.ModeHTML)
}

// SendEventRemoved sends a message the event is probably removed.
func SendEventRemoved(c tele.Context) error {
	return c.Send(locale.ResultEventRemoved, tele.RemoveKeyboard, tele.NoPreview, tele.ModeHTML)
}

// btnSignupScene creates reply buttons for the signup scene.
